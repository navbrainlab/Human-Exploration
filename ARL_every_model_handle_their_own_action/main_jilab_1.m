    ...
    for trial=1: trials
        ...
        if trial ==1
            ...
            for i = 1:NumModel
                ...
                % chose_action = randi([1,num_action])*i;
            ...
            action_idx = randi([1,all_num_action]);  % 从 A 中随机取 1 个
        ...
        [action_idx,state_of_all_model, q_of_model, p_of_model,all_RPE_of_model,all_RPE_uncertainty_of_model,which_model_was_choosen,lamda_of_model_all] = model_RL_9states_004(trial,dim,action_idx,state_of_all_model,p_of_model,q_of_model,lamda_of_model_all,alpha_of_model_all,all_RPE_of_model,all_RPE_uncertainty_of_model,score);
...
for agent=1:100
    ...
    choosemodelTable = all_model_choose_list;
    actionTable = all_agent_action_list;

    
    outputData = table;
    choices_ = choiceTable{agent, 1};  % 这个是 9x50 的 cell 数组
    choices_ = choices_{1};  % 提取具体的 9x50 数据
    
    % 获取对应的分数 (1x50 double array)
    scores = scoreTable(agent, :);  % 这个是 1x50 的 double 数组
    choosemodels = choosemodelTable(agent,:);
    actions = actionTable(agent,:);
    
    % 创建一个空的 cell 数组来存储 agentData
    agentData = [];  % 50 轮
    
    for roundIdx = 1:trials
        % 将每轮的 9 个选择与分数作为一个字符串存储
        strArray = cell(9, 1);
        this_round_food_sets = choices_(:,roundIdx);
        for i = 1:9
            % 将每个数组转换为字符串
            this_food_set = this_round_food_sets(i);
            this_food_set = this_food_set{1};
            strArray{i} = mat2str(this_food_set);
        ...
        newRow = cell2table({finalStr, scores(roundIdx),choosemodels(roundIdx),actions(roundIdx)}, 'VariableNames', {'Choice', 'Score','choose_model','Choose_model_action'});
        % agentData(roundIdx, :) = newRow;
        agentData = [agentData;newRow];

    end
    writetable(agentData,['C:\Users\Windows11\Desktop\3D_ARL_data\',int2str(agent),'_fitting.csv'])
end
